...
class Window(Frame):
    def __init__(self, canvas_frame, drawTk: ObjDrawTkinter, mg: ObjManager, layout_class: GraphLayout, master=None):
    ...
    def init_window(self):
        ...
        # canvas = Canvas(self, width=1200, height=1200,relief = RIDGE,bd=0, highlightthickness= 0)
        ...
        Edit.add_command(label="group_vertex", command=lambda: self.Groupvertex())
        Edit.add_command(label="bandwidth", command=lambda: self.Bandwidth())
        Edit.add_command(label="delay")  # , command = self.Delay)

        Layout_menu.add_command(label="reingold tilford circular", command=lambda: self.reingold_tilford_circular())
