...
class StatisticPie2(tk.Frame):
    def __init__(self, stat, attribute, hour):
        # tk.Frame.__init__(self, parent)
        # self.bins = 10
        # fig = Figure(figsize=(8, 4.5), dpi=200)
        # ax = fig.add_subplot(111)
        ...
        patches, texts = plt.pie(values, labels=keys, labeldistance=1.04, colors=colors, startangle=90)
        plt.legend(patches, labels, loc="best")
        plt.axis('equal')
        plt.title("Throughput of the edges in the hour {}".format(hour))
        plt.tight_layout()
        plt.show()
